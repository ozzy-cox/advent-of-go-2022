...
func calcManDist(x Point, y Point) int {
	return int(math.Abs(float64((y[0] - x[0]) + (y[1] - x[1]))))
}
...
func findMinTime(jetsCache *[][]Jet, gridCache *[]Grid, start Point, end Point, jets []Jet, round int, dimensions [2]int) int {
	min := math.MaxInt
...
func main() {
	...
	a := findMinTime(&jetsCache, &gridCache, start, end, jets, 1, dimensions)
	b := findMinTime(&jetsCache, &gridCache, end, start, jets, a, dimensions)
	c := findMinTime(&jetsCache, &gridCache, start, end, jets, b, dimensions)
