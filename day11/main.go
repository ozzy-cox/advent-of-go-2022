	...
	for i := 0; i < 20; i++ {
		for _, monke := range monkes {
			for len(*monke.items) > 0 {
				...
				panicOnError(err)
				intItemAfterCalculation := itemAfterCalculation.(int) / 3
				*monke.inspectionCount += 1

				if intItemAfterCalculation%monke.test == 0 {
					monkes[monke.throwToIfTrue].items.Push(intItemAfterCalculation)
				} else {
					monkes[monke.throwToIfFalse].items.Push(intItemAfterCalculation)
				}
			}
		}
	}
